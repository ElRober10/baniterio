...
class AuthControllerIT extends IntegrationTest {
    ...
    // 620330952, 679874772, 686072485. 700000000 no está sembrado.

    @Test
    void login_correcto_devuelve_token_y_usuario() {
        http.post().uri("/api/v1/auth/registro")
                .body(registroValido("620330952", "login-ok@x.com"))
                .exchange()
                .expectStatus().isCreated();

        @SuppressWarnings("unchecked")
        Map<String, Object> body = http.post().uri("/api/v1/auth/login")
                .body(Map.of("telefono", "620330952", "password", "secreto1"))
                .exchange()
                .expectStatus().isOk()
                .expectBody(Map.class)
                .returnResult().getResponseBody();
    ...
    void login_con_password_incorrecta_devuelve_401() {
        http.post().uri("/api/v1/auth/registro")
                .body(registroValido("679874772", "login-bad@x.com"))
                .exchange()
                .expectStatus().isCreated();

        @SuppressWarnings("unchecked")
        Map<String, Object> body = http.post().uri("/api/v1/auth/login")
                .body(Map.of("telefono", "679874772", "password", "otra-cosa"))
                .exchange()
    ...
    void el_token_del_login_vale_para_yo() {
        http.post().uri("/api/v1/auth/registro")
                .body(registroValido("686072485", "yo@x.com"))
                .exchange()
                .expectStatus().isCreated();

        @SuppressWarnings("unchecked")
        Map<String, Object> login = http.post().uri("/api/v1/auth/login")
                .body(Map.of("telefono", "686072485", "password", "secreto1"))
                .exchange()
                .expectStatus().isOk()
                .expectBody(Map.class)
                .returnResult().getResponseBody();
